...
""", categoryName: "Editing")
    ...
    static var parameterSummary: some ParameterSummary {
        Summary("Add \(\.$bookTitle) by \(\.$author) with \(\.$coverImage)") {
            \.$datePublished
            \.$isRead
        }
    }
    ...
    func perform() async throws -> some ReturnsValue<ShortcutsBookEntity> {
        ...
        do {
            let newBook = try BookManager.shared.addBook(title: "\(bookTitle)", author: "\(author)", datePublished: datePublished, coverImage: image, isRead: isRead)
